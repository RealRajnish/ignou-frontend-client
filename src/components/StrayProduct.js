  ...
  const clicked = async () => {
    ...
    if (userLoggedIn) {
      ...
      const userRes = window.confirm(
        `Are you sure you want to Adapt this cute ${breed}..!!`
      );
      if (userRes) {
        try {
          ...
          window.alert(
            `Dear ${customer_info.name} Your Request for ${title} ${breed} has been placed Successfully..!! Our team will contact you soon!!`
          );
          Navigate("/");
        } catch (error) {
          console.log(error);
